...
while True:
    ...
    if job_status.get("results_available"):
        ...
        for url in urls:
            get_file(
                url=url,
                output_file=f"{OUTPUT_DIR}/job_output_{start_date}_{end_date}_{idx}.ndjson.gz"
            )
            print(f"Result file for {start_date} to {end_date}: {OUTPUT_DIR}/job_output_{start_date}_{end_date}_{idx}.ndjson.gz")
            idx+=1
