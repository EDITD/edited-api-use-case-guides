...
def get_schema(schema, headers, attempt_number=1):
    """Uses a filterset id to programmatically pull a filterset from Edited Market for use in other API Calls

    Args:
        schema: valid values: searches, retailers, brands or details
        headers: A dictionary containing the value/key pair of x-api-key and your API KEY

    Returns:
        A dictionary containing the schema data returned from the API

    Raises:
        None
    """

    valid_schemas = ['searches', 'retailers', 'brands', 'details']

    if schema not in valid_schemas:
        logger.error (f"{schema} schema is not one of: searches, retailers, brands, details")
        exit()
    ...
    try:
        ...
        response = session.get(
            url=f"https://apis.edited.com/schema/v1/{schema}",
            headers=headers,
            timeout=(30, 60),
        )
        ...
        try:
            ...
            if attempt_number < 3:
                return get_schema(schema=schema, headers=headers, attempt_number=attempt_number + 1)
            raise
